...
RUN apt install -y fluxbox nano xterm supervisor x11vnc xvfb novnc websockify surf wmctrl dillo uzbl
...
RUN mkdir -p /app/.fluxbox && \
    echo "pkill uzbl; uzbl http://localhost:8188 &" > /app/.fluxbox/startup && \
