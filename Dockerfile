...
RUN apt install -y fluxbox nano xterm supervisor x11vnc xvfb novnc websockify luakit surf
...
RUN mkdir -p /app/.fluxbox && \
    echo "pkill luakit; luakit http://localhost:8188 &" > /app/.fluxbox/startup && \
    echo "exec fluxbox" >> /app/.fluxbox/startup

# Créer le fichier menu pour surf dans Fluxbox
RUN echo "[begin] (menu)" > /app/.fluxbox/menu && \
    echo "[exec] (comfyui) {luakit http://localhost:8188}" >> /app/.fluxbox/menu && \
