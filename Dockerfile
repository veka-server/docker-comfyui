...
RUN apt install -y fluxbox nano xterm supervisor x11vnc xvfb novnc websockify surf thunar notepadqq smplayer 
...
RUN echo "[begin] (menu)" > /app/.fluxbox/menu && \
    ...
    echo "[exec] (notepadqq) {notepadqq}" >> /app/.fluxbox/menu && \
    echo "[exec] (video) {smplayer}" >> /app/.fluxbox/menu && \
