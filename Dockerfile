...
RUN apt install -y fluxbox nano xterm supervisor x11vnc xvfb novnc websockify surf wmctrl midori 
...
RUN mkdir -p /app/.fluxbox && \
    echo "pkill midori; midori http://localhost:8188 &" > /app/.fluxbox/startup && \
